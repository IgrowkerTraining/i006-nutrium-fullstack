...
class PatientController {
  ...
  async upsertProfile(req, res) {
    try {
      const { birth_date, gender, health_goals, tag_ids } = req.body;

      // ── Validaciones ──────────────────────────────────────────
      const errors = [];

      if (!birth_date || String(birth_date).trim() === '') {
        errors.push({ field: 'birth_date', message: 'birth_date es requerido (formato YYYY-MM-DD)' });
      } else {
        const parsed = new Date(birth_date);
        if (isNaN(parsed.getTime())) {
          errors.push({ field: 'birth_date', message: 'birth_date debe ser una fecha válida (YYYY-MM-DD)' });
        }
      }

      if (tag_ids !== undefined && !Array.isArray(tag_ids)) {
        errors.push({ field: 'tag_ids', message: 'tag_ids debe ser un array de IDs numéricos' });
      }
      ...
      const { profile, created } = await patientService.upsertProfile(
        req.user.id,
        { birth_date, gender, health_goals },
        tag_ids || []
      );
